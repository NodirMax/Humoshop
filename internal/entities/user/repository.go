...
func GetUserFromDB(login string) (users []UserDatabase, err error) {
	rows, err := db.DB.Query("SELECT * FROM users WHERE login=$1", login)
	if err != nil {
		return
	}
	defer rows.Close()
...
func CreateUserDB(user CreateUserStruct) (err error) {
...
func UpdateUserDB(user UpdateUserStruct) (err error) {
...
func DeleteUserDB(user DeleteUserStruct) (err error) {
